/**
 * BRIQ v1.6.0 — Confidence-Aware Decisioning Engine
 *
...
export function evaluateConfidence(
  ...
  const financialChecks = [
    { field: state.financing.purchasePrice.value, label: "Purchase Price", weight: 10 },
    { field: state.rent.monthlyRent.value, label: "Monthly Rent", weight: 10 },
    { field: state.financing.arv.value, label: "After Repair Value", weight: 5 },
    { field: state.financing.interestRate.value, label: "Interest Rate", weight: 5 },
    { field: state.expenses.taxes.value, label: "Property Taxes", weight: 5 },
    { field: state.expenses.insurance.value, label: "Insurance", weight: 5 },
  ];
  ...
  const propertyChecks = [
    state.property.propertyType.value,
    state.property.yearBuilt.value,
    state.property.assessedValue.value,
  ];
  const propertyHits = propertyChecks.filter(v => v != null && v !== 0 && v !== "").length;
  maxScore += 15;
  score += Math.round((propertyHits / propertyChecks.length) * 15);

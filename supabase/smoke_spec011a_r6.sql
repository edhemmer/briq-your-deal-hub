...
begin
  begin
    ...
    if not exists(select 1 from public.contractiq_questions_report_definitions where id=r5_all_id and report_state='stale' and not is_current) then
      raise exception 'R5 question change did not leave all-questions definition stale'; end if;
    execute 'set local role authenticated';

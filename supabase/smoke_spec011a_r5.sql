...
declare
  ...
  r5_all_id uuid;
  r5_lender_id uuid;
  r5_transition_snapshot jsonb;
  r5_transition_full jsonb;
  r5_transition_summary jsonb;
  r5_transition_all jsonb;
  r5_transition_lender jsonb;
  r5_transition_attorney jsonb;
  r5_mutation jsonb;
  r5_version integer;
  r5_status_transition_passed boolean := false;
  r5_role_transition_passed boolean := false;
  r5_multirole_passed boolean := false;
  r5_failure jsonb;
...
begin
  begin
    ...
    insert into public.contract_questions(
      ...
      source_evidence_id,source_evidence_ids,source_anchor,contract_conflict_id,contract_conflict_version,
      ...
      'title:controlling-legal-description',repeat('1',64),'buyer','open','unresolved',evidence_id,array[evidence_id],
    ...
    insert into public.contract_questions(
      ...
      evidence_id,array[evidence_id],'{"kind":"exhibit","label":"Exhibit A"}'::jsonb,conflict_id,1,true,
      '{"fullReport":true,"summaryReport":true,"standaloneQuestionsReport":true,"roleExport":true}'::jsonb,
      ...
      evidence_id,array[evidence_id],'{"kind":"section","label":"Financing Contingency"}'::jsonb,null,null,false,
      '{"fullReport":true,"summaryReport":true,"standaloneQuestionsReport":true,"roleExport":true}'::jsonb,
      repeat('6',64),user_id,user_id);
    update public.contract_questions set contract_deadline_id=deadline_id,contract_deadline_version=1
      where id=financing_question_id;
    ...
    insert into public.contract_questions(
      id,workspace_id,contract_id,property_id,contract_version,question,recipient_role,priority,category,
      rationale,why_it_matters,semantic_key,deterministic_key,perspective,status,resolution_state,
      source_evidence_id,source_evidence_ids,source_anchor,professional_review_required,report_inclusion,content_hash,created_by,updated_by
    )
    select value.id,workspace_id,contract_id,property_id,1,value.question,value.recipient_role,
      ...
      'buyer','open','unresolved',solar_evidence_id,array[solar_evidence_id],
    ...
    r5_all := public.create_contractiq_questions_report_definition(snapshot_two_id,'all_questions',null,'{}'::jsonb,
      'contractiq-questions-report-template-v1','spec011a-r5-all','d1111111-1111-4111-8111-111111111111');
    r5_all_id := (r5_all ->> 'reportDefinitionId')::uuid;
    r5_retry := public.create_contractiq_questions_report_definition(snapshot_two_id,'all_questions',null,'{}'::jsonb,
      'contractiq-questions-report-template-v1','spec011a-r5-all-retry','d1111111-1111-4111-8111-111111111112');
    if r5_all_id is null or r5_retry ->> 'reportDefinitionId'<>r5_all_id::text
      or not (r5_retry->>'reused')::boolean then raise exception 'R5 all questions idempotency failed: %, %',r5_all,r5_retry; end if;
    r5_grouped := public.create_contractiq_questions_report_definition(snapshot_two_id,'grouped_by_role',null,'{}'::jsonb,
      'contractiq-questions-report-template-v1','spec011a-r5-grouped','d1111111-1111-4111-8111-111111111113');
    r5_attorney := public.create_contractiq_questions_report_definition(snapshot_two_id,'selected_role','buyer_attorney','{}'::jsonb,
      'contractiq-questions-report-template-v1','spec011a-r5-attorney','d1111111-1111-4111-8111-111111111114');
    r5_lender := public.create_contractiq_questions_report_definition(snapshot_two_id,'selected_role','lender','{}'::jsonb,
      'contractiq-questions-report-template-v1','spec011a-r5-lender','d1111111-1111-4111-8111-111111111115');
    ...
    if not exists(select 1 from public.contractiq_questions_report_definition_projection d where d.report_definition_id=r5_all_id
      ...
      or not exists(select 1 from public.contractiq_role_question_export_projection d
        where d.export_definition_id=(r5_attorney->>'reportDefinitionId')::uuid and d.selected_role='buyer_attorney'
          and not exists(select 1 from jsonb_array_elements(d.canonical_question_refs) q where q->>'targetRole'<>'buyer_attorney'))
      or not exists(select 1 from public.contractiq_role_question_export_projection d
        where d.export_definition_id=r5_lender_id and d.selected_role='lender'
          and exists(select 1 from jsonb_array_elements(d.canonical_question_refs) q where q->>'questionId'=solar_lender_question_id::text)
          and (d.counts->>'relevantDeadlineCount')::integer>=1
          and not exists(select 1 from jsonb_array_elements(d.canonical_question_refs) q where q->>'targetRole'<>'lender'))
      or not exists(select 1 from public.contractiq_role_question_export_projection d
        where d.export_definition_id=(r5_solar->>'reportDefinitionId')::uuid and d.selected_role='solar_provider'
          and exists(select 1 from jsonb_array_elements(d.canonical_question_refs) q where q->>'questionId'=solar_buyout_question_id::text)
          and not exists(select 1 from jsonb_array_elements(d.canonical_question_refs) q where q->>'targetRole'<>'solar_provider')) then
      raise exception 'R5 all/grouped/role filter projection failed'; end if;
    ...
    begin
      insert into public.contractiq_questions_report_definitions(workspace_id,deal_id,property_id,contract_id,perspective,
        ...
        template_version,report_mode,filter_rules,grouped_questions,canonical_question_refs,content_scope,counts,
        source_cutoff_at,content_hash,deterministic_definition_hash,report_state,reconciliation_state)
      values(workspace_id,deal_id,property_id,contract_id,'buyer',snapshot_two_id,2,repeat('a',64),repeat('b',64),
        1,99,'invalid','all_questions','{}','[]','[]','{}','{}',now(),repeat('c',64),repeat('d',64),'current','reconciled');
    exception when insufficient_privilege then direct_write_denied:=true; end;
    if not direct_write_denied then raise exception 'R5 direct write accepted'; end if;
    execute 'reset role';
    ...
    begin
      perform public.create_contractiq_questions_report_definition(snapshot_two_id,'selected_role','lender','{}',
        'contractiq-questions-report-template-v1','spec011a-r5-cross-workspace',gen_random_uuid());
    exception when insufficient_privilege then cross_workspace_denied:=true; end;
    if not cross_workspace_denied then raise exception 'R5 cross-workspace read accepted'; end if;
    execute 'reset role';
    perform set_config('request.jwt.claims',jsonb_build_object('sub',user_id,'role','authenticated')::text,true);
    execute 'set local role authenticated';
    begin
      select version into r5_version from public.contract_questions where id=financing_question_id;
      r5_mutation:=public.add_contractiq_question_response(financing_question_id,
        '{"response":"Financing terms are received for review.","responderRole":"lender","verificationState":"unverified"}'::jsonb,
        r5_version,'spec011a-r5-response',gen_random_uuid());
      if not exists(select 1 from public.contractiq_questions_report_definitions d where d.id=r5_all_id
        and d.report_state='stale' and not d.is_current) then raise exception 'R5 answer did not stale prior definition'; end if;
      r5_transition_snapshot:=public.create_contractiq_report_snapshot(contract_id,'buyer',analysis_id,
        'spec011a-r5-answered-snapshot',gen_random_uuid());
      r5_transition_full:=public.create_contractiq_full_report_definition((r5_transition_snapshot->>'snapshotId')::uuid,
        'contractiq-full-report-template-v1','spec011a-r5-answered-full',gen_random_uuid(),false);
      r5_transition_summary:=public.create_contractiq_buyer_summary_definition((r5_transition_full->>'reportDefinitionId')::uuid,
        'contractiq-buyer-summary-template-v1','spec011a-r5-answered-summary',gen_random_uuid(),false);
      r5_transition_all:=public.create_contractiq_questions_report_definition((r5_transition_snapshot->>'snapshotId')::uuid,
        'all_questions',null,'{}','contractiq-questions-report-template-v1','spec011a-r5-answered-all',gen_random_uuid());
      if not exists(select 1 from public.contractiq_questions_report_definitions d where d.id=(r5_transition_all->>'reportDefinitionId')::uuid
        and exists(select 1 from jsonb_array_elements(d.canonical_question_refs) q where q->>'questionId'=financing_question_id::text
          and q->>'status'='answered' and q->>'responseState'='received' and q->>'responseVerificationState'='unverified')) then
        raise exception 'R5 answered/unverified regeneration failed'; end if;
      select version into r5_version from public.contract_questions where id=financing_question_id;
      r5_mutation:=public.resolve_contractiq_question(financing_question_id,
        '{"resolutionState":"verified_resolved","verificationState":"verified"}'::jsonb,
        r5_version,'spec011a-r5-resolve',gen_random_uuid());
      r5_transition_snapshot:=public.create_contractiq_report_snapshot(contract_id,'buyer',analysis_id,
        'spec011a-r5-resolved-snapshot',gen_random_uuid());
      r5_transition_full:=public.create_contractiq_full_report_definition((r5_transition_snapshot->>'snapshotId')::uuid,
        'contractiq-full-report-template-v1','spec011a-r5-resolved-full',gen_random_uuid(),false);
      r5_transition_summary:=public.create_contractiq_buyer_summary_definition((r5_transition_full->>'reportDefinitionId')::uuid,
        'contractiq-buyer-summary-template-v1','spec011a-r5-resolved-summary',gen_random_uuid(),false);
      r5_transition_all:=public.create_contractiq_questions_report_definition((r5_transition_snapshot->>'snapshotId')::uuid,
        'all_questions',null,'{}','contractiq-questions-report-template-v1','spec011a-r5-resolved-all',gen_random_uuid());
      r5_transition_lender:=public.create_contractiq_questions_report_definition((r5_transition_snapshot->>'snapshotId')::uuid,
        'selected_role','lender','{}','contractiq-questions-report-template-v1','spec011a-r5-resolved-lender',gen_random_uuid());
      if not exists(select 1 from public.contractiq_questions_report_definitions d where d.id=(r5_transition_all->>'reportDefinitionId')::uuid
        and exists(select 1 from jsonb_array_elements(d.canonical_question_refs) q where q->>'questionId'=financing_question_id::text
          and q->>'status'='resolved' and q->>'resolutionState'='verified_resolved')) then
        raise exception 'R5 resolved regeneration failed'; end if;
      r5_status_transition_passed:=true;
      ...
      r5_mutation:=public.update_contractiq_canonical_question(solar_lender_question_id,
        '{"targetRole":"buyer_attorney"}'::jsonb,r5_version,'spec011a-r5-role-change',gen_random_uuid());
      if not exists(select 1 from public.contractiq_questions_report_definitions d
        where d.id=(r5_transition_lender->>'reportDefinitionId')::uuid and d.report_state='stale' and not d.is_current) then
        raise exception 'R5 old lender export not stale after role change'; end if;
      r5_transition_snapshot:=public.create_contractiq_report_snapshot(contract_id,'buyer',analysis_id,
        'spec011a-r5-role-snapshot',gen_random_uuid());
      r5_transition_full:=public.create_contractiq_full_report_definition((r5_transition_snapshot->>'snapshotId')::uuid,
        'contractiq-full-report-template-v1','spec011a-r5-role-full',gen_random_uuid(),false);
      r5_transition_summary:=public.create_contractiq_buyer_summary_definition((r5_transition_full->>'reportDefinitionId')::uuid,
        'contractiq-buyer-summary-template-v1','spec011a-r5-role-summary',gen_random_uuid(),false);
      r5_transition_lender:=public.create_contractiq_questions_report_definition((r5_transition_snapshot->>'snapshotId')::uuid,
        'selected_role','lender','{}','contractiq-questions-report-template-v1','spec011a-r5-role-lender',gen_random_uuid());
      r5_transition_attorney:=public.create_contractiq_questions_report_definition((r5_transition_snapshot->>'snapshotId')::uuid,
        'selected_role','buyer_attorney','{}','contractiq-questions-report-template-v1','spec011a-r5-role-attorney',gen_random_uuid());
      if exists(select 1 from public.contractiq_questions_report_definitions d
        cross join lateral jsonb_array_elements(d.canonical_question_refs) q
        where d.id=(r5_transition_lender->>'reportDefinitionId')::uuid and q->>'questionId'=solar_lender_question_id::text)
        or not exists(select 1 from public.contractiq_questions_report_definitions d
          cross join lateral jsonb_array_elements(d.canonical_question_refs) q
          where d.id=(r5_transition_attorney->>'reportDefinitionId')::uuid and q->>'questionId'=solar_lender_question_id::text
            and q->>'targetRole'='buyer_attorney') then raise exception 'R5 role reassignment filtering failed'; end if;
      r5_role_transition_passed:=true;
      select version into r5_version from public.contract_questions where id=solar_lender_question_id;
      r5_mutation:=public.update_contractiq_canonical_question(solar_lender_question_id,
        '{"reportInclusion":{"fullReport":true,"summaryReport":true,"standaloneQuestionsReport":true,"roleExport":true,"roleExportRoles":["lender"]}}'::jsonb,
        r5_version,'spec011a-r5-explicit-shared-role',gen_random_uuid());
      r5_transition_snapshot:=public.create_contractiq_report_snapshot(contract_id,'buyer',analysis_id,
        'spec011a-r5-shared-snapshot',gen_random_uuid());
      r5_transition_full:=public.create_contractiq_full_report_definition((r5_transition_snapshot->>'snapshotId')::uuid,
        'contractiq-full-report-template-v1','spec011a-r5-shared-full',gen_random_uuid(),false);
      r5_transition_summary:=public.create_contractiq_buyer_summary_definition((r5_transition_full->>'reportDefinitionId')::uuid,
        'contractiq-buyer-summary-template-v1','spec011a-r5-shared-summary',gen_random_uuid(),false);
      r5_transition_lender:=public.create_contractiq_questions_report_definition((r5_transition_snapshot->>'snapshotId')::uuid,
        'selected_role','lender','{}','contractiq-questions-report-template-v1','spec011a-r5-shared-lender',gen_random_uuid());
      if not exists(select 1 from public.contractiq_questions_report_definitions d
        cross join lateral jsonb_array_elements(d.canonical_question_refs) q
        where d.id=(r5_transition_lender->>'reportDefinitionId')::uuid and q->>'questionId'=solar_lender_question_id::text
          and q->>'targetRole'='buyer_attorney' and q->>'displayRole'='lender'
          and q->>'response' is null and d.content_scope->>'responseVisibilityLevel'='state_only'
          and (d.content_scope->>'privateBuyerNotesExcluded')::boolean) then
        raise exception 'R5 explicit multi-role or private response scope failed'; end if;
      r5_multirole_passed:=true;
      select version into r5_version from public.contract_questions where id=solar_buyout_question_id;
      r5_mutation:=public.update_contractiq_canonical_question(solar_buyout_question_id,
        '{"targetRole":"seller","reportInclusion":{"fullReport":true,"summaryReport":true,"standaloneQuestionsReport":true,"roleExport":true,"professionalOnly":true}}'::jsonb,
        r5_version,'spec011a-r5-professional-scope',gen_random_uuid());
      select version into r5_version from public.contract_questions where id=financing_question_id;
      r5_mutation:=public.resolve_contractiq_question(financing_question_id,
        '{"resolutionState":"superseded"}'::jsonb,r5_version,'spec011a-r5-historical',gen_random_uuid());
      r5_transition_snapshot:=public.create_contractiq_report_snapshot(contract_id,'buyer',analysis_id,
        'spec011a-r5-history-snapshot',gen_random_uuid());
      r5_transition_full:=public.create_contractiq_full_report_definition((r5_transition_snapshot->>'snapshotId')::uuid,
        'contractiq-full-report-template-v1','spec011a-r5-history-full',gen_random_uuid(),false);
      r5_transition_summary:=public.create_contractiq_buyer_summary_definition((r5_transition_full->>'reportDefinitionId')::uuid,
        'contractiq-buyer-summary-template-v1','spec011a-r5-history-summary',gen_random_uuid(),false);
      r5_transition_all:=public.create_contractiq_questions_report_definition((r5_transition_snapshot->>'snapshotId')::uuid,
        'all_questions',null,'{"includeHistorical":true}'::jsonb,'contractiq-questions-report-template-v1',
        'spec011a-r5-history-all',gen_random_uuid());
      if not exists(select 1 from public.contractiq_questions_report_definitions d
        where d.id=(r5_transition_all->>'reportDefinitionId')::uuid
          and (d.counts->>'historicalQuestionCount')::integer>=1
          and (d.counts->>'totalCurrentQuestions')::integer+(d.counts->>'historicalQuestionCount')::integer
            =jsonb_array_length(d.canonical_question_refs)
          and exists(select 1 from jsonb_array_elements(d.canonical_question_refs) q
            where q->>'questionId'=financing_question_id::text and (q->>'historical')::boolean)) then
        raise exception 'R5 current/historical count or superseded labeling failed'; end if;
      r5_historical_count_passed:=true;
      begin
        perform public.create_contractiq_questions_report_definition((r5_transition_snapshot->>'snapshotId')::uuid,
          'selected_role','seller','{}','contractiq-questions-report-template-v1',
          'spec011a-r5-professional-only-denial',gen_random_uuid());
      exception when insufficient_privilege then r5_professional_denied:=true; end;
      if not r5_professional_denied then raise exception 'R5 professional-only content leaked to seller export'; end if;
      raise exception using errcode='BR005',message='SPEC011A_R5_TRANSITIONS_ROLLBACK';
    exception when sqlstate 'BR005' then null; end;
    if not r5_status_transition_passed or not r5_role_transition_passed or not r5_multirole_passed
      or not r5_historical_count_passed or not r5_professional_denied then
      raise exception 'R5 transition smoke incomplete'; end if;
    ...
    if not exists(select 1 from public.contractiq_questions_report_definitions where id=r5_all_id and report_state='stale' and not is_current)
      or not exists(select 1 from public.contractiq_questions_report_definitions where id=r5_lender_id and report_state='stale' and not is_current) then
      raise exception 'R5 question change did not stale all and role definitions'; end if;
    execute 'set local role authenticated';
    r5_failure:=public.create_contractiq_questions_report_definition(snapshot_two_id,'all_questions',null,'{}',
      'contractiq-questions-report-template-v1','spec011a-r5-stale-regeneration',gen_random_uuid());
    r5_failure_retry:=public.create_contractiq_questions_report_definition(snapshot_two_id,'all_questions',null,'{}',
      'contractiq-questions-report-template-v1','spec011a-r5-stale-regeneration',gen_random_uuid());
    if r5_failure->>'reportState'<>'failed_with_prior_valid'
      or r5_failure->>'priorValidDefinitionId'<>r5_all_id::text
      or r5_failure_retry->>'reportDefinitionId'<>r5_failure->>'reportDefinitionId'
      or not (r5_failure_retry->>'reused')::boolean
      or not exists(select 1 from public.contractiq_questions_report_definitions d
        where d.id=r5_all_id and d.report_state='stale')
      or not exists(select 1 from public.domain_events e
        where e.entity_id=(r5_failure->>'reportDefinitionId')::uuid and e.event_type='contractiq.questions_report_definition_failed') then
      raise exception 'R5 failure recovery or retry failed: %, %',r5_failure,r5_failure_retry; end if;
  ...
  if exists(select 1 from public.contractiq_full_report_definitions where contract_id=contract_id)
     or exists(select 1 from public.contractiq_questions_report_definitions where contract_id=contract_id)
  ...
  insert into spec011a_r4_smoke_result(test_name,passed,detail) values
    ('r5_questions_and_role_exports',true,jsonb_build_object('allDefinitionId',r5_all_id,
      ...
      'idempotentRetry',r5_retry->>'reused','staleAfterChange',true,'statusTransition',r5_status_transition_passed,
      'roleTransition',r5_role_transition_passed,'explicitMultiRole',r5_multirole_passed,
      'historicalCounts',r5_historical_count_passed,'professionalOnlyDenied',r5_professional_denied,
      'failureWithPriorValid',r5_failure->>'priorValidPreserved','failureRetry',r5_failure_retry->>'reused',
      'directWriteDenied',direct_write_denied,'crossWorkspaceDenied',cross_workspace_denied,'fixtureRollbackVerified',true)),

create temporary table spec011a_r3_smoke_result (
...
begin
  begin
    insert into auth.users (
      ...
      ('00000000-0000-0000-0000-000000000000',other_user_id,'authenticated','authenticated','spec011a-r3-other@example.invalid','',now(),'{}','{}',now(),now());
    ...
    insert into public.contract_deadline_results(
      ...
      repeat('d',64),true,'spec011a-r3-deadline',now(),user_id
    );
    ...
    snapshot_one := public.create_contractiq_report_snapshot(
      contract_id,'buyer',analysis_id,'spec011a-r3-snapshot-1','cd111111-1111-4111-8111-111111111111'
    );
    ...
    definition_one := public.create_contractiq_full_report_definition(
      snapshot_one_id,'contractiq-full-report-template-v1','spec011a-r3-definition-1',
    ...
    snapshot_two := public.create_contractiq_report_snapshot(
      contract_id,'buyer',analysis_id,'spec011a-r3-snapshot-2','cd111111-1111-4111-8111-111111111112'
    );
    ...
    definition_two := public.create_contractiq_full_report_definition(
      snapshot_two_id,'contractiq-full-report-template-v1','spec011a-r3-definition-2',
    ...
    snapshot_three := public.create_contractiq_report_snapshot(
      contract_id,'buyer',analysis_id,'spec011a-r3-snapshot-3','cd111111-1111-4111-8111-111111111113'
    );
    snapshot_three_id := (snapshot_three ->> 'snapshotId')::uuid;
    failure_result := public.create_contractiq_full_report_definition(
      snapshot_three_id,'contractiq-full-report-template-v1','spec011a-r3-definition-failure',
    ...
    reconcile_result := public.reconcile_contractiq_full_report_definition(definition_two_id,'spec011a-r3-reconcile-1');
    ...
    begin
      perform public.create_contractiq_full_report_definition(
        snapshot_three_id,'contractiq-full-report-template-v1','spec011a-r3-cross-workspace',
    ...
    raise exception using errcode='BR003',message='SPEC011A_R3_ROLLBACK';
  ...
  insert into spec011a_r3_smoke_result(test_name,passed,detail)
  values('full_due_diligence_report_definition',true,jsonb_build_object(
...
select * from spec011a_r3_smoke_result;

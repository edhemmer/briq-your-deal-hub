-- Keep verified routine terms in transaction terms, not in the decision issue list.
  ...
  begin
    ...
    from (select value item from jsonb_array_elements(summary_items) value
      where value ->> 'materiality' in ('material','critical')
        and (
          (value ->> 'itemType'='finding' and value #>> '{canonicalContent,status}' in ('current','open','needs_review'))
          or (value ->> 'itemType'='conflict' and value #>> '{canonicalContent,resolutionState}' in ('unresolved','under_review','professional_review_required'))
          or (value ->> 'itemType'='question' and value #>> '{canonicalContent,status}' in ('open','in_progress','blocked'))
          or (value ->> 'itemType'='open_item' and value #>> '{canonicalContent,status}' in ('Open','In Progress'))
          or (value ->> 'itemType'='deadline' and value #>> '{canonicalContent,status}' in ('urgent','missed','uncertain'))
          or value ->> 'evidenceClassification' in ('supported_concern','open_question')
          or value ->> 'fullSectionId' in ('solar-service','ownership-exposure')
        )
    ...
    select item into obligation from jsonb_array_elements(summary_items) item
      ...
      order by case item ->> 'fullSectionId' when 'solar-service' then 0 else 1 end,
        case item ->> 'itemType' when 'term' then 0 else 1 end,
...
-- Solar agreements are primary obligations before the generic term classifiers run.
create or replace function public.contractiq_full_report_section_for(item_kind text, item jsonb)
returns text language plpgsql stable parallel safe set search_path=public,pg_temp as $$
declare
  classifier text := lower(concat_ws(' ', item ->> 'category', item ->> 'termType',
    item ->> 'findingGroup', item ->> 'findingType', item ->> 'conflictType',
    item ->> 'targetDomain', item ->> 'title', item ->> 'itemType'));
begin
  ...
  if item_kind='term' and classifier ~ '(solar|battery|ppa|service agreement)' then return 'solar-service'; end if;

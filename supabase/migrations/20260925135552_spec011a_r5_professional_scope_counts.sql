create or replace function public.generate_contractiq_questions_report_definition(
...
declare
  ...
  role_list jsonb; recipient text; item jsonb; requested_historical boolean;
  total_count integer:=0; unresolved_count integer:=0; answered_count integer:=0;
...
begin
  ...
  for question_item in select value from jsonb_array_elements(coalesce(snap.snapshot_payload->'questions','[]'::jsonb)) loop
    ...
    if target_mode='selected_role' and question_item->>'recipientRole'<>target_role and not role_list ? target_role then
      continue; end if;
    if target_mode='selected_role'
      and coalesce((question_item#>>'{reportInclusion,professionalOnly}')::boolean,false)
      and target_role not in ('buyer_attorney','seller_attorney','title_company','lender','insurer',
        'inspector','appraiser','specialist','contractor') then
      raise exception 'Professional-only question cannot enter this role export.' using errcode='42501';
    end if;
  ...
  counts:=jsonb_build_object('totalCurrentQuestions',total_count-
      (select count(*) from jsonb_array_elements(refs) v where (v->>'historical')::boolean),
    'historicalQuestionCount',(select count(*) from jsonb_array_elements(refs) v where (v->>'historical')::boolean),
    'unresolved',unresolved_count,'openQuestionCount',

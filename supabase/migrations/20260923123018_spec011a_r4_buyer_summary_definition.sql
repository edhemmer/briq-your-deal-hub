-- Specification 011A R4. Summary content is a projection of one R3 definition
-- and its frozen R1 snapshot. No report-owned calculations or question state.
create table public.contractiq_buyer_summary_definitions (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces(id) on delete cascade,
  deal_id uuid not null,
  property_id uuid not null,
  contract_id uuid not null,
  perspective text not null default 'buyer' check (perspective = 'buyer'),
  snapshot_id uuid not null,
  snapshot_version integer not null check (snapshot_version > 0),
  snapshot_hash text not null check (snapshot_hash ~ '^[0-9a-f]{64}$'),
  full_report_definition_id uuid not null,
  full_report_definition_version integer not null check (full_report_definition_version > 0),
  full_report_content_hash text not null check (full_report_content_hash ~ '^[0-9a-f]{64}$'),
  analysis_version integer not null check (analysis_version > 0),
  summary_definition_version integer not null check (summary_definition_version > 0),
  definition_contract_version text not null default 'contractiq-buyer-summary-definition-v1',
  template_version text not null default 'contractiq-buyer-summary-template-v1',
  ...
  recommendation_state text check (recommendation_state in (
  ...
  top_issue_count integer not null default 0 check (top_issue_count >= 0),
  material_question_count integer not null default 0 check (material_question_count >= 0),
  open_item_count integer not null default 0 check (open_item_count >= 0),
  critical_deadline_count integer not null default 0 check (critical_deadline_count >= 0),
  content_hash text check (content_hash is null or content_hash ~ '^[0-9a-f]{64}$'),
  deterministic_definition_hash text check (deterministic_definition_hash is null or deterministic_definition_hash ~ '^[0-9a-f]{64}$'),
  ...
  unique (workspace_id,contract_id,perspective,summary_definition_version),
  ...
  foreign key (workspace_id,full_report_definition_id) references public.contractiq_full_report_definitions(workspace_id,id) on delete restrict,
  foreign key (workspace_id,superseded_by_definition_id) references public.contractiq_buyer_summary_definitions(workspace_id,id)
);

create unique index idx_contractiq_summary_snapshot_full_template
  on public.contractiq_buyer_summary_definitions(workspace_id,snapshot_id,full_report_definition_id,template_version)
  where summary_state <> 'failed_with_prior_valid';
create unique index idx_contractiq_summary_current
  on public.contractiq_buyer_summary_definitions(workspace_id,contract_id,perspective)
  where is_current;
create index idx_contractiq_summary_history
  on public.contractiq_buyer_summary_definitions(workspace_id,contract_id,generated_at desc);
create index idx_contractiq_summary_full_fk
  on public.contractiq_buyer_summary_definitions(workspace_id,full_report_definition_id);
create index idx_contractiq_summary_snapshot_fk
  on public.contractiq_buyer_summary_definitions(workspace_id,snapshot_id);
create index idx_contractiq_summary_created_by_fk
  on public.contractiq_buyer_summary_definitions(created_by) where created_by is not null;
create index idx_contractiq_summary_superseded_by_fk
  on public.contractiq_buyer_summary_definitions(workspace_id,superseded_by_definition_id)
  where superseded_by_definition_id is not null;

create function public.protect_contractiq_buyer_summary_content()
returns trigger language plpgsql set search_path = public, pg_temp as $$
begin
  if old.summary_state <> 'generating' and (
    ...
    or new.perspective is distinct from old.perspective or new.snapshot_id is distinct from old.snapshot_id
    or new.snapshot_version is distinct from old.snapshot_version or new.snapshot_hash is distinct from old.snapshot_hash
    or new.full_report_definition_id is distinct from old.full_report_definition_id
    or new.full_report_definition_version is distinct from old.full_report_definition_version
    or new.full_report_content_hash is distinct from old.full_report_content_hash
    or new.analysis_version is distinct from old.analysis_version
    or new.summary_definition_version is distinct from old.summary_definition_version
    ...
    raise exception 'Completed Buyer Summary content is immutable. Create a new definition version.' using errcode='55000';
  end if;
  new.updated_at := now();
  return new;
end; $$;

create function public.reconcile_contractiq_buyer_summary_definition(target_definition_id uuid,idempotency_key text)
returns jsonb language plpgsql security definer set search_path=public,pg_temp as $$
#variable_conflict use_variable
declare
  target_definition public.contractiq_buyer_summary_definitions%rowtype;
...
begin
  ...
  select * into target_definition from public.contractiq_buyer_summary_definitions where id=target_definition_id;
  if target_definition.id is null or not public.is_workspace_member(target_definition.workspace_id) then
    raise exception 'Buyer Summary definition not found.' using errcode='42501';
  end if;
  if not public.has_workspace_permission(target_definition.workspace_id,'deals:manage') then
    raise exception 'You do not have permission to reconcile Buyer Summaries.' using errcode='42501';
  end if;
  command := public.ensure_contract_command(target_definition.workspace_id,target_definition.deal_id,
    target_definition.property_id,target_definition.contract_id,'reconcile_contractiq_buyer_summary_definition',
    idempotency_key,jsonb_build_object('summaryDefinitionId',target_definition.id,'contentHash',target_definition.content_hash));
  if command.result ? 'summaryDefinitionId' then return command.result || jsonb_build_object('reused',true); end if;
  select * into full_definition from public.contractiq_full_report_definitions
    where workspace_id=target_definition.workspace_id and id=target_definition.full_report_definition_id;
  select * into target_snapshot from public.contractiq_report_snapshots
    where workspace_id=target_definition.workspace_id and id=target_definition.snapshot_id;
  mismatch := full_definition.id is null or target_snapshot.id is null
    or full_definition.report_definition_version is distinct from target_definition.full_report_definition_version
    or full_definition.content_hash is distinct from target_definition.full_report_content_hash
    or full_definition.report_state <> 'current' or not full_definition.is_current
    or target_snapshot.snapshot_version is distinct from target_definition.snapshot_version
    or target_snapshot.content_hash is distinct from target_definition.snapshot_hash
    or target_snapshot.snapshot_state in ('stale','superseded')
    or target_snapshot.reconciliation_status <> 'reconciled'
    or full_definition.recommendation_state is distinct from target_definition.recommendation_state
    or full_definition.source_cutoff_at is distinct from target_definition.source_cutoff_at
    or full_definition.analysis_version is distinct from target_definition.analysis_version
    or target_definition.definition_payload #>> '{identity,fullReportDefinitionId}' is distinct from full_definition.id::text
    or target_definition.definition_payload #>> '{identity,snapshotId}' is distinct from target_snapshot.id::text;
  if mismatch and target_definition.is_current then
    update public.contractiq_buyer_summary_definitions set summary_state='stale',is_current=false,
      reconciliation_state='stale',stale_reason='full_or_snapshot_reconciliation_mismatch'
      where id=target_definition.id;
  end if;
  update public.contract_command_requests set result=jsonb_build_object(
    'summaryDefinitionId',target_definition.id,'summaryDefinitionVersion',target_definition.summary_definition_version,
    'summaryState',case when mismatch then 'stale' else target_definition.summary_state end,
    'contentHash',target_definition.content_hash,'deterministicDefinitionHash',target_definition.deterministic_definition_hash,
    'validationEligible',not mismatch and coalesce((target_definition.validation_result ->> 'eligible')::boolean,false),
    'failureCode',case when mismatch then 'summary_reconciliation_mismatch' else null end)
    where id=command.id;
  return (select result from public.contract_command_requests where id=command.id) || jsonb_build_object('reused',false);
end; $$;

create function public.stale_contractiq_buyer_summaries_on_full_change()
returns trigger language plpgsql security definer set search_path=public,pg_temp as $$
declare changed_definition record;
begin
  if new.report_state in ('stale','superseded','failed_with_prior_valid')
    and (old.report_state is distinct from new.report_state or old.is_current is distinct from new.is_current) then
    for changed_definition in
      update public.contractiq_buyer_summary_definitions summary
      set summary_state='stale',is_current=false,reconciliation_state='stale',
        stale_reason=coalesce(new.stale_reason,'full_report_definition_stale')
      where summary.workspace_id=new.workspace_id and summary.full_report_definition_id=new.id
        and summary.is_current
      returning summary.*
    loop
      insert into public.domain_events (workspace_id,deal_id,property_id,actor_id,event_type,entity_type,
        entity_id,entity_version,source_command,idempotency_key,correlation_id,payload)
      values (changed_definition.workspace_id,changed_definition.deal_id,changed_definition.property_id,auth.uid(),
        'contractiq.summary_definition_stale','contractiq_buyer_summary_definition',changed_definition.id,
        changed_definition.summary_definition_version,'contractiq_full_report_definition_state_change',
        'buyer-summary-stale:' || changed_definition.id || ':' || new.report_definition_version,
        new.correlation_id,jsonb_build_object('summaryDefinitionId',changed_definition.id,
          'fullReportDefinitionId',new.id,'reason',coalesce(new.stale_reason,'full_report_definition_stale')));
    end loop;
  end if;
  return new;
end; $$;
create trigger stale_contractiq_buyer_summaries_on_full_change
after update of report_state,is_current,stale_reason on public.contractiq_full_report_definitions
for each row execute function public.stale_contractiq_buyer_summaries_on_full_change();

create trigger protect_contractiq_buyer_summary_content before update
  on public.contractiq_buyer_summary_definitions for each row
  execute function public.protect_contractiq_buyer_summary_content();

alter table public.contractiq_buyer_summary_definitions enable row level security;
create policy "contractiq buyer summaries read authorized workspace"
  on public.contractiq_buyer_summary_definitions for select to authenticated
  using (public.is_workspace_member(workspace_id));
create policy "contractiq buyer summaries no direct insert"
  on public.contractiq_buyer_summary_definitions for insert to authenticated with check (false);
create policy "contractiq buyer summaries no direct update"
  on public.contractiq_buyer_summary_definitions for update to authenticated using (false) with check (false);
create policy "contractiq buyer summaries no direct delete"
  on public.contractiq_buyer_summary_definitions for delete to authenticated using (false);

create view public.contractiq_buyer_summary_definition_projection with (security_invoker=true) as
select definition.id as summary_definition_id,definition.summary_definition_version,
  definition.definition_contract_version,definition.template_version,definition.workspace_id,
  definition.deal_id,definition.property_id,definition.contract_id,definition.perspective,
  definition.snapshot_id,definition.snapshot_version,definition.snapshot_hash,
  definition.full_report_definition_id,definition.full_report_definition_version,
  definition.full_report_content_hash,definition.analysis_version,definition.summary_state,
  definition.recommendation_state,definition.reconciliation_state,definition.source_cutoff_at,
  definition.stale_reason,definition.definition_payload,definition.validation_result,
  definition.top_issue_count,definition.material_question_count,definition.open_item_count,
  definition.critical_deadline_count,definition.content_hash,definition.deterministic_definition_hash,
  definition.is_current,definition.generated_at,definition.generation_duration_ms,
  coalesce((select jsonb_agg(jsonb_build_object(
    'summaryDefinitionId',history.id,'summaryDefinitionVersion',history.summary_definition_version,
    'snapshotId',history.snapshot_id,'fullReportDefinitionId',history.full_report_definition_id,
    'summaryState',history.summary_state,'contentHash',history.content_hash,'generatedAt',history.generated_at
  ) order by history.summary_definition_version desc)
  from public.contractiq_buyer_summary_definitions history
  where history.workspace_id=definition.workspace_id and history.contract_id=definition.contract_id), '[]'::jsonb) as history
from public.contractiq_buyer_summary_definitions definition;

create function public.create_contractiq_buyer_summary_definition(
...
begin
  ...
  begin
    ...
    where question ->> 'status' in ('open','in_progress','blocked')
      and (question ->> 'priority' in ('critical','high') or coalesce((question ->> 'professionalReviewRequired')::boolean,false));
    select coalesce(jsonb_agg(jsonb_build_object(
    ...
    payload := jsonb_build_object(
      ...
      'primaryLongTermObligation',obligation,'materialPropertyFindings',property_findings,
...
grant select on public.contractiq_buyer_summary_definitions,public.contractiq_buyer_summary_definition_projection to authenticated;
revoke all on public.contractiq_buyer_summary_definitions,public.contractiq_buyer_summary_definition_projection from anon;
revoke insert,update,delete on public.contractiq_buyer_summary_definitions from authenticated;
revoke all on function public.create_contractiq_buyer_summary_definition(uuid,text,text,uuid,boolean) from public,anon;
grant execute on function public.create_contractiq_buyer_summary_definition(uuid,text,text,uuid,boolean) to authenticated;
revoke all on function public.reconcile_contractiq_buyer_summary_definition(uuid,text) from public,anon;
grant execute on function public.reconcile_contractiq_buyer_summary_definition(uuid,text) to authenticated;
